  ...
  return (
    <main className="site-main page-v2">
      <PageHero
        ...
        highlights={["External music links", "Art gallery", "Spoken word", "Artist spotlights", "Creative campaigns"]}
        media={stockMedia.homeStories[2]}
        asideTitle="Non-streaming, community-led"
        asideBody="All media is linked externally — YouTube, Audiomack, Spotify — keeping the platform lightweight and accessible."
      />

      <Reveal as="section" delay={120}>
        <SectionIntro eyebrow="Listen and watch" title="Music and video linked from platforms communities already use." body="External links keep the site fast and accessible while connecting supporters to the creative work." />
        <div className="card-grid-v2 card-grid-v2--3">
          {musicLinks.map((link) => (
            <article key={link.platform} className="card-v2">
              ...
              <a href={link.href} target="_blank" rel="noreferrer" className="button button--secondary" style={{ justifySelf: "start" }}>{link.label}</a>
            </article>
          ))}
        </div>
      </Reveal>
      ...
      <Reveal as="section" delay={180}>
        ...
        <div className="split-v2">
          <StockPhoto src={stockMedia.homeStories[2].src} alt={stockMedia.homeStories[2].alt} label="Community storytelling" sizes="(max-width: 1120px) 100vw, 48vw" />
        ...
        <div className="split-v2" style={{ marginTop: "0.85rem" }}>
          <StockPhoto src={stockMedia.educationFeature.src} alt={stockMedia.educationFeature.alt} label="Education access" sizes="(max-width: 1120px) 100vw, 48vw" />
          <StockPhoto src={stockMedia.homeHero.src} alt={stockMedia.homeHero.alt} label="Youth development" sizes="(max-width: 1120px) 100vw, 48vw" />
        </div>
      </Reveal>
      ...
      <Reveal as="section" className="dark-panel-v2" delay={360}>
        ...
        <p className="dark-panel-v2__body">Artists, musicians, filmmakers, and storytellers can submit work for review and inclusion in campaigns, galleries, and advocacy projects.</p>
        <div className="hero-actions">
          <LoadingLink href="/get-involved" className="button button--primary" loadingLabel="Opening">Submit creative work</LoadingLink>
          <LoadingLink href="/programs" className="button button--secondary" loadingLabel="Opening">Back to programs</LoadingLink>
        </div>
      </Reveal>
    </main>
  );
