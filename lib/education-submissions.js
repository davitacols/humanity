...
let educationTablesReady = false;

// Self-provisioning: the contribute flow works whether or not the full
// seed-neon.js script has been run. Schema matches scripts/seed-neon.js.
async function ensureEducationTables(sql) {
  if (educationTablesReady) {
    return;
  }
...
export async function createEducationSubmission(data) {
  ...
  await ensureEducationTables(sql);
