...
const inquiryStatusSet = new Set(["pending", "contacted", "resolved", "archived"]);
...
export async function updateSupportInquiryStatus(id, nextStatus) {
  const sql = getSql();
  ...
  if (!Number.isFinite(inquiryId) || !inquiryStatusSet.has(normalizedStatus)) {
  ...
  try {
    ...
    return {
      metrics: [
        ...
        { value: String(countRow.contacted), label: "contacted and awaiting next step" },
        { value: String(countRow.resolved), label: "resolved conversations" },
